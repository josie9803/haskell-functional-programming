...
push (State(stack, bool)) x = 
   if x `elem` stack
   then State(stack, bool)
   else State(x:stack, bool)
...
pop (State([],bool)) = (Nothing,State([],bool))
pop (State(h:t,bool)) 
   | bool == False = (Nothing,State(h:t,bool))
   | otherwise = (Just h,State(t,bool))
...
setInactive (State(stack,bool)) = State(stack,False)
...
setActive (State(stack,bool)) = State(stack,True)
...
mapState f (State(stack,bool)) = 
   State(nub (map f stack),bool)
...
popWhere _ (State(stack,False)) = (stack,State(stack,False))
popWhere pred (State(stack,True)) = 
   let 
      elements = filter pred stack 
      newStack = filter (not . pred) stack
   in (elements,State(newStack,True))